...
const T Heap<T>::peek(){
    //YOUR CODE HERE
    if (count == 0) throw std::underflow_error("empty heap");
...
void Heap<T>::remove(T item, void (*removeItemData)(T)){
    //YOUR CODE HERE
    int found = getItem(item);
    if (found == -1) return;

    if (removeItemData) removeItemData(elements[found]);
    if (found == 0){
        pop();
        return;
    }
    int x = (count - 1 ) - (found +1) + 1;
    memcpy(&elements[found], &elements[found +1], x*sizeof(T));
    count --;
    int start = found;
    int last = count - 1;
    count = found;
...
bool Heap<T>::contains(T item){
    ...
    for (int i = 0; i < count; i++ ){
        if (compare(item, elements[i]) == 0) return true;
    }
    return false;
}
...
void Heap<T>::heapify(T array[], int size){
    //YOUR CODE HERE
    for (int i = 0; i < size; i++) this->push(array[i]);
}
...
void Heap<T>::reheapUp(int position){
    ...
    int x = (position - 1) /2;
    if (aLTb(this -> elements [position], this -> elements[x])){
        this -> swap (position,x);
        reheapUp(x);
    }
}
...
void Heap<T>::reheapDown(int position){
    ...
    int LC = position*2 + 1;
    int RC = position*2 + 2;
    int lastP = this -> count -1;

    if (LC <= lastP){
        int smallC = LC;
        if (RC <= lastP){
            if (aLTb(this -> elements[LC], this -> elements[RC])) smallC = LC;
            else smallC = RC;
        }
        if (aLTb(this ->elements[smallC], this ->elements[position])){
            this -> swap (smallC, position);
            reheapDown(smallC);
        }
    }
}
