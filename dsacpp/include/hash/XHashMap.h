...
V XHashMap<K,V>::put(K key, V value){
    //YOUR CODE HERE
    int address = this -> hashCode(key, capacity);
    Entry* entry = table[address];
    ...
    Entry* newE = new Entry(key, value, 0, table[address]);
    if (table[address] != 0) table[address] -> prev =newE;
    table[address]  = newE;
    count++;
...
V& XHashMap<K,V>::get(K key){
    //YOUR CODE HERE
    int address = hashCode(key, capacity);
    Entry *entry = table [address];
