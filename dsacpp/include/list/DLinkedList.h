...
void DLinkedList<T>::add(int index, T e) {
    ...
    Node* prevN = this->getPreviousNodeOf(index);
    node->next=prevN->next;
    prevN->next->prev=node;
    node->prev=prevN;
    prevN->next=node;
...
T DLinkedList<T>::removeAt(int index){
    ...
    //YOUR CODE HERE
        Node *prevN = getPreviousNodeOf(index);
        Node *todelete = prevN->next;
        T item = todelete->data;
        prevN->next = todelete->next;
        todelete->next->prev = prevN;
        delete todelete;
        count--;
        return item;
}
...
bool DLinkedList<T>::contains(T item){
    //YOUR CODE HERE
        ...
        while (temp != this->tail){
            if (this->equals(temp->data, item, itemEqual)) return true;
            temp = temp->next;
        }
        return false;
}
